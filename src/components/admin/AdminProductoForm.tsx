...
export default function AdminProductoForm({ producto, categorias, esNuevo, colores }: Props) {
  ...
  // Extras asignados
  type ExtraConImg = { id: string; name: string; description: string; price: number; imageUrl?: string };
  const [extrasAsignados, setExtrasAsignados]   = useState<string[]>([]);
  const [extrasDisponibles, setExtrasDisponibles] = useState<ExtraConImg[]>([]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              return (
                ...
                >
                  <input
                    ...
                    className="rounded accent-colonta-primary shrink-0"
                  />
                  {/* Imagen del extra */}
                  <div className="w-10 h-10 rounded-lg shrink-0 overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center">
                    {extra.imageUrl ? (
            ...
            {extrasDisponibles.length === 0 && (
              <p className="text-sm text-slate-400 col-span-2">
                No hay extras creados aún. Crea extras desde{" "}
                <a href="/admin/extras" className="text-colonta-primary underline">Admin → Extras</a>.
              </p>
            )}
          </div>
        </div>
